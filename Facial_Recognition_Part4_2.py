        ...
        try:            
            ...
            if confidence > 75:
                ...
                if (datetime.datetime.now() - start_time).seconds < seconds_to_run:
                    #timestamp = time.time()
                    #formatted = time.strftime('%Y년 %m월 %d일 %M분 %S초',time.localtime(timestamp))
                    
                    nowtime = datetime.datetime.now().strftime('%Y년 %m월 %d일 %M분 %S초')                
                    print (nowtime,"유사도는", confidence, "%입니다. 유사도가 75% 이하이므로 본인이 아닙니다.")
                    
                    #time.sleep(1)                    
            #75 이하면 타인.. Locked!!! 
                #cv2.putText(image, "Locked", (250, 450), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
                #cv2.imshow('Face Cropper', image)
                #print ("유사도는 ", confidence, "%이다. 유사도 75% 이하이므로 본인 아닙니다.")
                else:
                    print (nowtime,"\n제한시간을 초과하였으므로 프로그램을 종료합니다.")
                    break                
        except:
            if (datetime.datetime.now() - start_time).seconds < seconds_to_run:
            #얼굴 검출 안됨 
            ...
            #cv2.imshow('Face Cropper', image)
                ...
                # 년 월 일 분 초
                nowtime = datetime.datetime.now().strftime('%Y년 %m월 %d일 %M분 %S초')                
                print (nowtime,"얼굴 인식에 실패했습니다. 초점을 맞춰 주세요.")
                #time.sleep(1)
                pass
