...
def run(models):    
    ...
    fail_count = 0      #유사도 75이하 카운트 수
    no_count = 0        #얼굴 검출 실패 카운트
    endTime = time.time() + 30 #30초 동안 동작
    ...
    while True:
        ...
        try:            
            ...
            if confidence > 75:
                ...
                #유사도 검출 실패 30회 초과 시 처리과정
                if fail_count<30:
                    fail_count+=1
            #75 이하면 타인.. Locked!!! 
                #cv2.putText(image, "Locked", (250, 450), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
                #cv2.imshow('Face Cropper', image)
                    print ("실패 횟수는 : ",fail_count,". 유사도는 ", confidence, "%이다. 유사도 75% 이하이므로 본인 아닙니다.")
                    
                else:
                    print (fail_count," 유사도가 75이하가 30회 이상이므로 종료합니다.")
                    break
                
        except:
            #얼굴 검출 실패 30회 초과 시 처리과정
            if no_count<30:
                no_count+=1
            #얼굴 검출 안됨 
            #cv2.putText(image, "Face Not Found", (250, 450), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 0, 0), 2)
            #cv2.imshow('Face Cropper', image)
                print ("얼굴 검출 실패",no_count,"회")
                pass
            else:
                print ("얼굴 검출이",no_count,"회 실패하였으므로 종료합니다.")
                break
